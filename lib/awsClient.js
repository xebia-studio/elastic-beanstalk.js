...
function AWSClient(options) {
  ...
  // If credentials are provided use it. By default use SDK configuration
  if(options.credentials) {
    AWS.config.credentials = options.credentials;
  }

  if(options.httpOptions) {
    AWS.config.httpOptions = options.httpOptions;
  }
  ...
  this.prefixBucket = options.prefixBucket;
...
AWSClient.prototype.getApplicationVersion = function (version) {
  var self = this;
  var defered = Q.defer();
  ...
  if (version) {
    options.VersionLabels = [version];
  }

  self.elasticbeanstalk.describeApplicationVersions(options, function (err, result) {
    if (err) {
      defered.reject(err);
      return;
    }

    defered.resolve(result.ApplicationVersions);
  });
  return defered.promise;
};
...
AWSClient.prototype.uploadArchiveToS3 = function (fileName, remoteFilename) {
  ...
  var options = {
    ...
    Key: (self.prefixBucket || '') + remoteFilename,
    Body: fileBuffer
  };
...
AWSClient.prototype.createApplicationVersion = function (versionLabel, description, remoteFilename) {
  ...
  var options = {
    ...
    SourceBundle: {
      ...
      S3Key: (self.prefixBucket || '') + remoteFilename
    }
  };
